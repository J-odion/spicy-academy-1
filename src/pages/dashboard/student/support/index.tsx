    ...
    <DashboardSidebar>
      <div className="flex justify-between gap-10">
        <div className="w-full mt-20 md:mt-20">
          <h1>Support</h1>
          <p>Commonly asked question</p>
          <Accordion type="single" collapsible>
            {currentItems.map((faq) => (
              <AccordionItem key={faq.id} value={faq.id.toString()}>
                <AccordionTrigger className="text-[#1E1E1E] text-sm fornt-normal">{faq.question}</AccordionTrigger>
                <AccordionContent className="text-[#976B5B]">{faq.answer}</AccordionContent>
              </AccordionItem>
            ))}
            </Accordion>
        </div>
